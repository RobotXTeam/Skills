...
- `scripts/package-agent-deb.sh`: build agent-only CLI deb for headless Ubuntu/Debian devices.
...
Linux agent-only packaging:

- `packaging/linux/linkbit-agent.service`: systemd unit for `/usr/bin/linkbit-agent`.
- `packaging/linux/linkbit-agent-configure`: root helper to write `/etc/linkbit/agent.env` and enable/restart the service.
