...
- Conda env: `VoxAI`
- Python: `/home/steven/miniconda3/envs/VoxAI/bin/python`
- Whisper command wrapper: `/home/steven/.local/bin/whisper`
- Real Whisper binary: `/home/steven/miniconda3/envs/VoxAI/bin/whisper`
- FFmpeg: `/usr/bin/ffmpeg`
- Runtime: CPU only at last setup check (`torch.cuda.is_available() == False`)

Do not create a new conda environment unless this one is broken. The `whisper` command is already on PATH and points directly to the `VoxAI` env binary.
...
Default `/VoxAI <media>` behavior summarizes the whole meeting as project-state deltas. If the user invokes `/VoxAI <media> me`, or asks to "只记录我的部分", "只记录 Steven/我的发言", "识别我的声音", or similar, use speaker-aware mode:
...
- Current known `VoxAI` environment has `openai-whisper`, CPU `torch`, `torchaudio` CPU wheel, `speechbrain`, `soundfile`, and `scikit-learn`.
- The local speaker clustering helper is `/home/steven/VoxAI/tools/diarize_me.py`.
- Steven's default reference voice sample is `/home/steven/VoxAI/录音/张强声音.m4a` (about 14.4 seconds). Use it automatically for `/VoxAI <media> me` when present.
...
ALL_PROXY=socks5://127.0.0.1:7897/ all_proxy=socks5://127.0.0.1:7897/ \
  /home/steven/miniconda3/envs/VoxAI/bin/python /home/steven/VoxAI/tools/diarize_me.py <media> \
  --me-audio /home/steven/VoxAI/录音/张强声音.m4a --output <workdir>/diarization.json
```
...
- Use `/home/steven/VoxAI/tools/diarize_me.py` first for `/VoxAI <media> me`.
- If `/home/steven/VoxAI/录音/张强声音.m4a` exists, use it with `--me-audio` for `/VoxAI <media> me`.
...
2. `/home/steven/VoxAI/memory`, if present.
...
```text
/home/steven/VoxAI/输出/YYYY-MM-DD.md
```

Rules:

- Create `/home/steven/VoxAI/输出` if it does not exist.
...
For `/VoxAI <media> me`, write the user-facing Markdown output document to:

```text
/home/steven/VoxAI/输出/YYYY-MM-DD-me.md
```

Speaker-aware `me` output rules:

- Do not overwrite `/home/steven/VoxAI/输出/YYYY-MM-DD.md` unless the user explicitly asks to replace the full-meeting document.
...
5. **Write memory updates**
   - For normal mode:
     - Create or update `/home/steven/VoxAI/输出/YYYY-MM-DD.md` as the required user-facing output document.
     ...
     - Verify that `/home/steven/VoxAI/输出/YYYY-MM-DD.md`, `memory/summaries/YYYY-MM-DD.md`, and `memory/summaries/latest.md` exist after writing and contain the same content.
   ...
   - For speaker-aware `me` mode:
     - Create or update `/home/steven/VoxAI/输出/YYYY-MM-DD-me.md`.
...
```markdown
文档已输出：/home/steven/VoxAI/输出/YYYY-MM-DD.md
...
For `/VoxAI <media> me`, the final response must first state:

```markdown
文档已输出：/home/steven/VoxAI/输出/YYYY-MM-DD-me.md
