...
description: >
  ...
  【默认行为】无参数调用时，执行 reCamera 舆情监控（最近 2 个月 Twitter + Reddit）。
...
当用户输入 `/agent-reach` 且不带任何参数时，执行 **reCamera 舆情监控**：

1. 读取 [references/recamera-monitor.md](references/recamera-monitor.md) 了解完整流程
2. 拉取最近 2 个月的 Twitter + Reddit 数据
3. 区分 Seeed 官方 vs 非官方讨论
4. 按标准格式输出舆情报告

如有参数（如「帮我搜小红书深圳美食」），走下面的路由表。
...
| reCamera 舆情监控 | — | [references/recamera-monitor.md](references/recamera-monitor.md) |

## 代理配置（国内网络环境）

访问 Twitter/Reddit 等被墙平台时，需设置代理。检测到 Clash Verge (mihomo) 时自动使用：

```bash
# 检测代理端口（优先 7897，其次 7890）
PROXY_PORT=$(ss -tlnp 2>/dev/null | grep -oP ':(7897|7890)' | head -1 | tr -d ':')
...
# 用法示例
proxy_cmd twitter search "query" -n 10
proxy_cmd rdt search "query" -c
```

> **注意**：不要修改用户全局 `.zshrc`/`.bashrc`。代理仅在 skill 命令中按需注入。
...
- [reCamera 舆情监控](references/recamera-monitor.md) — 默认行为，Twitter + Reddit 月报
