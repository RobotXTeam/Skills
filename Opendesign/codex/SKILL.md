...
- 当用户直接使用 `$opendesign` 或等价触发这个 skill，但没有补充任何具体要求时，默认执行“更新桌面版到主仓库最新 `main`”。
- 默认命令：
  ```bash
  ...
  ```
- 默认目标：
  - 拉取 `~/work/cli/OpenDesign/open-design` 的最新 `main`
  - 保留并恢复本地 OpenDesign 品牌定制
  - 重新编译 Linux AppImage
  - 更新 `~/.local/bin/OpenDesign.default.AppImage`
  - 更新 `~/.local/share/OpenDesign/app/squashfs-root/`
  - 更新 `~/.local/share/applications/open-design-default.desktop`
- 默认执行后最少核验：
  ```bash
  git -C ~/work/cli/OpenDesign/open-design rev-parse --short HEAD
  stat ~/.local/bin/OpenDesign.default.AppImage
  stat ~/.local/share/OpenDesign/app/squashfs-root/AppRun
  stat ~/.local/share/applications/open-design-default.desktop
  ```
...
- 如果用户只触发 `$opendesign` 然后直接回车，没有写额外内容，就按“更新桌面版到主仓库最新 `main`”处理。
- 不先问“你是要更新、启动还是排错”。
- 只有当用户明确说“不要更新”“只启动”“只解释”“只看代码”时，才偏离这个默认动作。
...
```bash
source ~/.nvm/nvm.sh && nvm use 24 && corepack enable && corepack prepare pnpm@10.33.2 --activate
...
- 更新前先临时 `stash` 品牌相关 tracked 文件，避免 `git pull` 因本地改动被阻塞
- 用 `sed` 把名称改回 "OpenDesign"
- 从 `assets/icon.*` 恢复自定义图标
- 重建 `tools-pack` 包使名称生效

### 6. `corepack` / `pnpm` 版本飘到 11.x

**现象**:
- `corepack` 提示下载 `pnpm 11.x`
- 实际 `pnpm --version` 不是项目要求的 `10.33.2`
...
`update-and-build.sh` 已内置这一步。

### 7. `git fetch` / `git pull` 过程中 TLS 断流
...
**原因**:
- GitHub 拉取大对象时网络层中断，不是代码冲突。

**解决**:
```bash
git -C ~/work/cli/OpenDesign/open-design -c http.version=HTTP/1.1 -c http.maxRequests=1 fetch --progress origin main
git -C ~/work/cli/OpenDesign/open-design merge --ff-only origin/main
cd ~/work/cli/OpenDesign
./update-and-build.sh
```
...
- **默认空输入动作就是更新桌面版**，不是输出帮助菜单
- **不要使用根目录的 `pnpm build` 或 `pnpm test`**，此项目要求包级命令
- **不要推送代码到远程**，我们只是消费者
- **`node_modules/` 和 `.tmp/` 已 gitignore**，不会影响 `git pull`
- **品牌层本地改动主要在 4 个 tracked 文件**：`tools/pack/src/linux.ts`、`tools/pack/resources/linux/open-design.desktop.template`、`tools/pack/resources/linux/icon.png`、`tools/pack/resources/mac/icon.png`
