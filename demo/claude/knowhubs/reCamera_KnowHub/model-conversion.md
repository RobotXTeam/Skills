...
- 将工作保持在任务目录下，如 `$DEMO_DIR/recamera_<model>`。
...
- 使用的源检查点：`<download_dir>/depth_anything_v2_vits.pth`。
- 工作目录：`$DEMO_DIR/recamera_depth_anything`。
- `224x224` BF16 编译成功但在 reCamera 上失败，错误 `ion ioctl fail:: Out of memory`。
- `168x168` BF16 运行，NPU 时间约 `656 ms`。
- `224x224` INT8 使用 8 张图片的最小校准集运行：模型 `$DEMO_DIR/recamera_depth_anything/models/depth_anything_v2_vits_224_int8_min8.cvimodel`，单张图片约 `1494 ms`，实时摄像头 `1693 ms/frame`。
- 报告/证据：`$DEMO_DIR/recamera_depth_anything/DEPLOY_REPORT.md`，`<evidence_dir>/recamera_live_depth_anything_224_int8_jpeg.mp4`。
...
- 在 reCamera 上保存帧，拉回到主机，然后用 `ffmpeg` 编码。
...
cd $REPO_ROOT/solutions/<demo_name>
