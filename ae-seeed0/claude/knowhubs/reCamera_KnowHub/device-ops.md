...
- 网络拓扑和各入口 IP（reCamera USB/LAN、seeed0 各网段）：见 `environments/seeed0-recamera/network.md`。
- SSH 用户/密码：见 `environments/seeed0-recamera/credentials.md`；优先使用封装脚本自动尝试，必要时用 `RECAMERA_PASSWORD` 显式指定。
- USB 动态 IP：在 seeed0 上用 `environments/seeed0-recamera/scripts/seeed0_usb_ip.sh` 探测，禁止硬编码旧值（迁移过渡期设备可能仍物理连在旧主机上，一律假设经 seeed0 访问）。

## SSH 与文件传输

通过 `seeed0` 跳板访问 reCamera，统一使用封装脚本（自动依次尝试 reCamera 密码，密码见 `environments/seeed0-recamera/credentials.md`，可用 `RECAMERA_PASSWORD` 显式指定）：

```bash
environments/seeed0-recamera/scripts/recamera_ssh.sh 'hostname; whoami'
environments/seeed0-recamera/scripts/recamera_scp_to.sh <local-file> /home/recamera/<demo>/
```
...
对 reCamera Web UI 截图时，通过 `seeed0` 做 SSH 本地转发：

```bash
ssh -N -L 18080:192.168.42.1:80 seeed0   # seeed0 密码见 environments/seeed0-recamera/credentials.md
```
...
这些来自 `RobotXTeam/sscma-example-sg200x` 发布 `v1.0.1`，也存在于 seeed0 的仓库中（`/home/seeed0/sscma-example-sg200x`）。
