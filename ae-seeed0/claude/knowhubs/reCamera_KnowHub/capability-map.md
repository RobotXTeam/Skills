...
- 备份目录：`~/work/reCamera_demo/<demo_name>/`（从 seeed0 同步来的备份）
- Claude skills：`~/.claude/skills/ae-seeed0/`
...
- 代理：无需任何代理（GitHub 直连可用；seeed0 上没有也不需要 Clash，git 不要配置 http.proxy）
...
所有 reCamera C++ demo 的代码编写和编译都在 `seeed0` 主机上。开发完成后，将 seeed0 上的工作目录同步到 Steven 本机（作为备份），然后在 seeed0 上推送到 GitHub：

```bash
# 1. seeed0 同步到本机（备份）
rsync -avz --exclude='.git' --exclude='*.cvimodel' --exclude='*.onnx' \
  seeed0:~/reCamera_demo/<demo_name>/ ~/work/reCamera_demo/<demo_name>/
...
```bash
SEEED0_USB_IP="$(environments/seeed0-recamera/scripts/seeed0_usb_ip.sh)"
...
- 默认仓库：`seeed0:/home/seeed0/sscma-example-sg200x`。
