...
Steven's active SG200X/reCamera C++ demo repository — the single source of truth for both editing and building — lives on the `seeed0` host:

```text
seeed0:/home/seeed0/sscma-example-sg200x
```

All reCamera demo code (the local repo) and all builds happen here. SSH into `seeed0` and edit/build the repository there; there is no separate local-PC copy.
...
Demos are always placed in `solutions/sesg-project/<demo_name>` — there is no other canonical demo location on `seeed0`.
...
- uncommitted files in `seeed0:/home/seeed0/sscma-example-sg200x`;
...
For model demos, keep generated models, complete evidence images, and evidence videos outside of the GitHub source tree by default (no `.cvimodel`, `.onnx`, `.pth`, `.pt`, full evidence sets, evidence videos, or large runtime libraries in git). Publish them to the fixed Google Drive directories (`run/`, `model/`, `evidence/image/`, `evidence/video/` under `agent:reCamera_Shared/Wiki/<demo_name>/`) and document the fixed Wiki root public link, exact child paths, and exact file names in README/Wiki. Do not use GitHub Release/LFS as the default publishing path. Full directory constants and upload commands: see `environments/seeed0-recamera/network.md` (云端资产发布).

## Post-Push Clean Verification

After pushing a demo from `seeed0` to GitHub, verify the pushed version before finalizing the Wiki: clean clone -> build with public command -> `file` check -> pull Drive assets -> deploy to reCamera -> run and collect evidence. If any step fails, the demo is not complete — return to the repository, fix, push again, and repeat until the full loop passes.

The canonical 7-step procedure and failure handling live in `environments/seeed0-recamera/development-policy.md` (Post-Push Verification Gate).

## Public Documentation Path Rule

README, Wiki, and demo docs are written for other developers, not only for Steven's local machine. Never include local absolute paths (`/home/steven/...`, `/home/seeed0/...`); use relative paths or portable variables (`$REPO_ROOT`, `$SDK_ROOT`, `$TOOLCHAIN_BIN`, `$DEMO_DIR`, `<path-to-model>`). Real local validation paths belong in internal deployment reports only. The full rule set is in `templates/wiki-output.md` (关键规则 2：路径处理).
