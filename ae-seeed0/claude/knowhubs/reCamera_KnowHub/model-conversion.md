...
- Docker 镜像：`sophgo/tpuc_dev:v3.1`（seeed0 上已加载，容器 `tpuc_dev` 已用 `docker create` 建立并固定挂载 `-v /home/seeed0/tpu-workspace:/workspace`，可 `sudo docker start -ai tpuc_dev` 直接进入；也可按下方模板 `sudo docker run --rm` 临时起）。注意：seeed0 用户不在 docker 组（`/var/run/docker.sock` 为 root:docker 660），docker 命令一律加 `sudo`；seeed0 在 sudo 组，密码 0。
...
- 在 reCamera 上保存帧，将其拉回到 seeed0（scp/sshpass），然后使用 `ffmpeg` 编码。
...
在 seeed0 上构建（`~/.bashrc`/`~/.profile` 已写入这些变量，交互式 shell 生效；脚本/非交互命令中仍应显式写完整路径 export）：
