...
| 方法 | 本机收到帧率 | 丢帧 | 说明 |
|---|---|---|---|
| HTTP multipart MJPEG + seeed0 TCP 中继 | **~0.4 fps** | ~75% | mjpeg_server **轮询**最新 jpg（不是推），每 tick glob+stat 全目录，TCP sendall 背压 |
...
3.【中继】(fallback): 设备 udp_sender -> seeed0 USB IP:9100 ; seeed0 udp_relay 9100->本机:9200 ; 本机 udp_viewer :9200
4.【SSH 隧道桥】(PC 与 seeed0 之间只有单向 NAT/ssh 可达时): 设备 sender -> seeed0 USB:9100;
   seeed0 udp2tcp(UDP:9100 -> 长度前缀帧写 TCP 127.0.0.1:9101); 本机 `ssh -L 9101:127.0.0.1:9101 seeed0`;
   本机 tcp2udp(读隧道帧 -> sendto 127.0.0.1:9200); viewer 不变。 Linkbit 等单向打通环境见见 2026-08 实例。
```

reCamera 常见情况：USB 连 seeed0（192.168.42.x，只能经 seeed0 中继）；同时很多 reCamera 还有 wifi/LAN 口在你本机同子网（如 192.168.2.x）——优先用同子网直推。
...
2. **PMTU 黑洞**：设备经网关路由到本机时，TCP 三次握手能通（小包），但大段数据被丢（"TCP connects but data doesn't flow"）。表现：直连 LAN IP 的 8123 端口 curl 能连上但拿不到 body。**小 UDP 包(≤1400) 能穿过去**，但大 TCP 段不行——这是用 UDP 而非 TCP 直连的另一个理由；或走 seeed0 USB 中继绕开。
...
- `environments/seeed0-recamera/scripts/udp_sender.py` — 设备侧：监控 demo 输出目录，分片直推。
- `environments/seeed0-recamera/scripts/udp_viewer.py` — 本机侧：重组 + keep-latest + imdecode + imshow 窗口。
- `environments/seeed0-recamera/scripts/udp_relay.py` — seeed0 侧 fallback：UDP 数据包透传（设备经 USB 到 seeed0，再到本机）。
...
如果设备只能经 seeed0 USB（不同子网）：

```bash
# seeed0 起中继
python3 udp_relay.py 9100 192.168.2.101 9200   # seeed0 USB IP 用 environments/seeed0-recamera/scripts/seeed0_usb_ip.sh 在 seeed0 上探测（禁止硬编码旧值）
# 设备 sender 指向 seeed0 USB IP:9100
sudo python3 udp_sender.py /home/recamera/<demo>/live <seeed0_usb_ip> 9100
# 本机 viewer 不变
```
