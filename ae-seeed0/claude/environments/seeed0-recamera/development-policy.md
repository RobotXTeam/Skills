# seeed0 Host Development Policy
...
- SSH into `seeed0`.
- Create, edit, move, delete, build, and package demo code on `seeed0`.
...
`gh` is installed on `seeed0` and already logged in as `congchin38-coder` (credentials in `~/.config/gh/hosts.yml`); `gh auth setup-git` has been run. GitHub is reachable directly from seeed0 — no proxy of any kind, and never set `git config http.proxy` / `https.proxy`. The repository remote is currently an https URL with an embedded PAT (carried over in the migration); keep it as-is.
...
After a demo is finished and Steven approves the effect, commit and push from `seeed0` directly to GitHub. Do not push demo work before Steven approval.
...
2. Confirm the clean clone commit is the commit just pushed from `seeed0`.
