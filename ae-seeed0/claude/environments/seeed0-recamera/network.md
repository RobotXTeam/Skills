...
本环境于 2026-08-13 从旧部署主机迁移到新主机 `seeed0`，本文档所有 IP、路径、凭据均以 seeed0 实测为准。seeed0 双盘分工：
...
- PC -> `seeed0`：SSH 别名 `seeed0`（Steven 本机 `~/.ssh/config` 已配置，且已加 Steven 公钥免密），用户 `seeed0`，密码 `0`。
- seeed0 LAN：`192.168.2.194`。
- seeed0 OS：Ubuntu 24.04.3 LTS x86_64；Python 3.12.3（系统级，无 conda）。
...
- `seeed0` -> reCamera：USB/LAN `192.168.42.1`，设备 Web UI `http://192.168.42.1`。
...
- 迁移过渡期注意：reCamera 设备可能还物理连在旧主机上，但本 skill 一律假设设备经 seeed0 访问。
- seeed0 上还跑着 1Panel/openclaw 等容器，与 reCamera 工作流无关，不要去动。

## GitHub 直连，无需代理

seeed0 访问 GitHub 直连可用，无需任何代理：

- seeed0 上没有也不需要 Clash 或任何其它代理软件。
...
重要：seeed0 在 `192.168.42.0/24` 上的 USB IP 在 reCamera 重启或 USB 重连后会变化。永远不要硬编码旧值如 `192.168.42.197`。总是在 seeed0 上探测：

```bash
environments/seeed0-recamera/scripts/seeed0_usb_ip.sh
```

## 工作目录结构

### seeed0 主机环境（工作产物：机械盘）
...
│   ├── eval/                            ← 质量基准评测产物
│   │   ├── input/                       ← 固定评测输入（图片/视频/manifest）；seeed0 无 GPU，输入应精简
│   │   ├── baseline/                    ← seeed0 CPU Python baseline 输出
...
执行顺序（以下 rclone 命令全部在 **Steven 本机**执行；rclone 与 `agent:` remote 只配置在 Steven 本机，seeed0 上没有安装 rclone，也没有该 remote）：
...
`run/` 开箱即跑包的内容规格见 `environments/seeed0-recamera/development-policy.md`（run/ Ready-to-Run Package）。
