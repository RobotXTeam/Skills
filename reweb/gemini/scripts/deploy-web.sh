...
# The served root /usr/share/supervisor/www is a mode-dependent symlink. The
# Studio panel lives in the real directory below; deploy-web swaps that target.
# The symlink itself is owned by main.sh _panel_link / S93 and is never touched.
WEB_LINK=/usr/share/supervisor/www
WEB_STUDIO_DIR=/usr/share/supervisor/www-studio
...
printf '  device web dir:   %s (symlink %s untouched)\n' "$WEB_STUDIO_DIR" "$WEB_LINK"
...
note "step 3: backup current Studio web dir"
recamera_sudo_sh "mkdir -p '$BACKUP_DIR' && { [ -d '$WEB_STUDIO_DIR' ] && cp -a '$WEB_STUDIO_DIR' '$BACKUP_DIR/www' || mkdir -p '$BACKUP_DIR/www'; }" || die "backup failed"
...
note "step 5: swap Studio web dir (symlink is mode-owned, left untouched)"
# Legacy-layout migration first: on devices where /usr/share/supervisor/www is
# still the real Studio directory, promote it to www-studio and install the
# mode symlink, matching the current layout before swapping.
recamera_sudo_sh "if [ -d '$WEB_LINK' ] && [ ! -L '$WEB_LINK' ]; then rm -rf '$WEB_STUDIO_DIR.mig' 2>/dev/null; mv '$WEB_LINK' '$WEB_STUDIO_DIR.mig' 2>/dev/null; rm -rf '$WEB_STUDIO_DIR' 2>/dev/null; mv '$WEB_STUDIO_DIR.mig' '$WEB_STUDIO_DIR' 2>/dev/null; chown -R recamera:recamera '$WEB_STUDIO_DIR' 2>/dev/null; rm -f '$WEB_LINK' 2>/dev/null; ln -s '$WEB_STUDIO_DIR' '$WEB_LINK' 2>/dev/null; fi" || warn "legacy web-root migration skipped (already migrated or not applicable)"
swap_cmd="rm -rf '$WEB_STUDIO_DIR.failed' && { [ -d '$WEB_STUDIO_DIR' ] && mv '$WEB_STUDIO_DIR' '$WEB_STUDIO_DIR.failed' || true; } && mv '$STAGE_DIR' '$WEB_STUDIO_DIR' && chown -R recamera:recamera '$WEB_STUDIO_DIR' && rm -rf '$WEB_STUDIO_DIR.failed'"
recamera_sudo_sh "$swap_cmd" || {
  warn "swap failed; restoring backup"
  recamera_sudo_sh "rm -rf '$WEB_STUDIO_DIR' && cp -a '$BACKUP_DIR/www' '$WEB_STUDIO_DIR' && rm -rf '$STAGE_DIR'" || die "ROLLBACK FAILED: restore $WEB_STUDIO_DIR from $BACKUP_DIR/www manually"
...
note "step 6: verify deployed assets and API JSON"
# The served panel depends on the run mode (symlink), so verify the deployed
# Studio files at their real path and the API on :80. In studio mode this is a
# full end-to-end check; in nodered mode the served panel is the factory UI,
# which is expected and not a failure.
DEPLOY_MODE="$(recamera_sudo_sh "cat '$MODE_FILE' 2>/dev/null | tr -d ' \t\r\n'" || true)"
web_health_ok() {
  ...
  # Deployed Studio index must reference hashed assets that exist on device.
  idx="$(recamera_ssh "cat '$WEB_STUDIO_DIR/index.html'" || true)"
  case "$idx" in
    *'src="'*'.js'*) ;;
    *) return 1 ;;
  esac
  ...
  [ -n "$refs" ] || return 1
  for asset in $refs; do
    recamera_ssh "test -f '$WEB_STUDIO_DIR$asset'" || return 1
  done
  # :80 must serve a 200 page (either panel).
  http_code="$(recamera_ssh "curl -sS --max-time 8 -o /dev/null -w '%{http_code}' http://127.0.0.1/" || true)"
...
if [ "$health_ok" -ne 1 ]; then
  ...
  recamera_sudo_sh "rm -rf '$WEB_STUDIO_DIR' && cp -a '$BACKUP_DIR/www' '$WEB_STUDIO_DIR'" || die "ROLLBACK FAILED: restore $WEB_STUDIO_DIR from $BACKUP_DIR/www manually"
  restore_mode "$BACKUP_DIR" || die "ROLLBACK FAILED: restore /userdata/local/apps/mode from $BACKUP_DIR/mode.before manually"
  die "health check failed; restored web backup"
fi

note "deployed. backup: $BACKUP_DIR"
note "health: Studio dir assets + /api/version verified (device mode: ${DEPLOY_MODE:-unknown})"
printf 'rollback command:\n  sudo rm -rf %s && sudo cp -a %s/www %s\n  sudo cp %s/mode.before /userdata/local/apps/mode\n' "$WEB_STUDIO_DIR" "$BACKUP_DIR" "$WEB_STUDIO_DIR" "$BACKUP_DIR"
