...
Production runs from the device origin and talks directly to device ports. Development deliberately uses same-origin Vite paths so the browser tool sandbox never needs direct access to the private device network.
