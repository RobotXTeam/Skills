...
- Confirm the dev server was started with `cwd: solutions/supervisor/www` (as in the `supervisor-webui` launch entry).
...
- If the UI shows a blank terminal, check browser console logs and network events to see whether the upgrade request was issued and whether it returned 101.
