...
3. Start or reuse Browser Preview configuration `supervisor-webui`.
...
3. Start tunnels if they are not already listening:
   ...
   ```
4. Start the frontend through `.claude/launch.json` with Browser Preview (`supervisor-webui`). Do not launch dev servers with a generic background shell when Browser Preview is available.
...
- Keep Browser Preview at its **native/adaptive desktop size**. Never leave it pinned to a fixed mobile, tablet, or custom viewport between sessions — a stale fixed viewport is the usual reason the preview "looks wrong" on a new `/reweb` session. Resize to native/desktop before measuring.
...
3. Resize the Browser Preview viewport down to ~1040px and ~900px. Confirm the player scales continuously and only stacks to one column when the **content container** (not the viewport) drops to ≤620px.
...
```bash
/home/steven/.claude/skills/reweb/scripts/deploy-web.sh --dry-run
```
...
- frontend: Vite production build plus Browser Preview interaction against real APIs/WS;
