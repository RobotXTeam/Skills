...
```bash
/home/steven/.codex/skills/reweb/scripts/preflight.sh
```

```bash
/home/steven/.codex/skills/reweb/scripts/start-tunnels.sh
```

The dev server matches the `supervisor-webui` entry in `.claude/launch.json`: `npm run dev -- --host 0.0.0.0` with `solutions/supervisor/www` as cwd.
...
- Addon control-script change (`rootfs/usr/share/supervisor/addons/*.sh`, e.g.
  recorder.sh NAS direct-write support): use the same
  `deploy-supervisor.sh --include-runtime` (it overlays the addons directory
  with backup + rollback); the new script takes effect when the add-on is
  next started — no service restart beyond the routine supervisor restart.
- Full firmware: not part of this routine. Stop and obtain explicit authorization plus a recovery path.

## Stop tunnels

```bash
/home/steven/.codex/skills/reweb/scripts/stop-tunnels.sh
```
