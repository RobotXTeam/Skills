...
- 备份目录：`~/work/reCamera_demo/<demo_name>/`（从 seeed-reserver 同步来的备份）
- Claude skills：`~/.claude/skills/ae-reserver/`

### seeed-reserver 部署主机环境

- 工作目录：`/home/seeed0/reCamera_demo/<demo_name>/`（软链接，实际位于机械盘 `/media/seeed/新加卷/reCamera/reCamera_demo`，只放大件数据）
- 仓库目录：`/home/seeed0/sscma-example-sg200x/solutions/sesg-project/<demo_name>/`（固态盘；仓库唯一真源，编辑+编译+提交）
- 远程：`https://github.com/RobotXTeam/sscma-example-sg200x.git`（当前仍是带 PAT 的 HTTPS URL，沿用，勿改动）
- 认证：GitHub CLI（用户 congchin38-coder，`gh auth setup-git` 已执行）
- 代理：无需任何代理（GitHub 直连可用；seeed-reserver 上没有也不需要 Clash，git 不要配置 http.proxy）

现有 demo：
- `/home/seeed0/reCamera_demo/depth_anything/` - Depth Anything 深度估计
- `/home/seeed0/reCamera_demo/ppocr_v4/` - PP-OCRv4 文字识别

### 同步流程

所有 reCamera C++ demo 的代码编写和编译都在 `seeed-reserver` 主机上。开发完成后，将 seeed-reserver 上的工作目录同步到 Steven 本机（作为备份），然后在 seeed-reserver 上推送到 GitHub：

```bash
# 1. seeed-reserver 同步到本机（备份）
rsync -avz --exclude='.git' --exclude='*.cvimodel' --exclude='*.onnx' \
  seeed-reserver:~/reCamera_demo/<demo_name>/ ~/work/reCamera_demo/<demo_name>/

# 2. seeed-reserver 上复制到仓库并推送
ssh seeed-reserver "
...
- 已知结果：修复后通过，评分 6/10。
- 资产：`/home/seeed0/sscma-example-sg200x/solutions/sesg-project/face_udp`。
- 官方接收器：`udp_receiver.py`。
- 工作命令模板：

```bash
SEEED0_USB_IP="$(environments/seeed-reserver-recamera/scripts/reserver_usb_ip.sh)"
sudo env LD_LIBRARY_PATH=/mnt/system/lib:/mnt/system/usr/lib:/mnt/system/usr/lib/3rd:$LD_LIBRARY_PATH \
  ...
  single 0.7 1 "$SEEED0_USB_IP" 5001 20
```
...
SEEED0_USB_IP="$(environments/seeed-reserver-recamera/scripts/reserver_usb_ip.sh)"
sudo env LD_LIBRARY_PATH=/mnt/system/lib:/mnt/system/usr/lib:/mnt/system/usr/lib/3rd:$LD_LIBRARY_PATH \
  ./recamera_benchmark ./yolo11n_detection_cv181x_int8.cvimodel "$SEEED0_USB_IP"
```
...
- 默认仓库：`seeed-reserver:/home/seeed0/sscma-example-sg200x`。
...
- 验证：构建/部署模型检测器、停止服务、使用环境运行、验证 `http://192.168.42.112/modeldetector` JSON。
