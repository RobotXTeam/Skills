...
Steven's active SG200X/reCamera C++ demo repository — the single source of truth for both editing and building — lives on the `seeed-reserver` host:

```text
seeed-reserver:/home/seeed0/sscma-example-sg200x
```

All reCamera demo code (the local repo) and all builds happen here. SSH into `seeed-reserver` and edit/build the repository there; there is no separate local-PC copy.
...
Demos are always placed in `solutions/sesg-project/<demo_name>` — there is no other canonical demo location on `seeed-reserver`.
...
- uncommitted files in `seeed-reserver:/home/seeed0/sscma-example-sg200x`;
