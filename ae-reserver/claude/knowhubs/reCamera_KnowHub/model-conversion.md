...
- Docker 镜像：`sophgo/tpuc_dev:v3.1`（seeed-reserver 上已加载，容器 `tpuc_dev` 已用 `docker create` 建立并固定挂载 `-v /home/seeed0/tpu-workspace:/workspace`，可 `sudo docker start -ai tpuc_dev` 直接进入；也可按下方模板 `sudo docker run --rm` 临时起）。注意：seeed-reserver 用户不在 docker 组（`/var/run/docker.sock` 为 root:docker 660），docker 命令一律加 `sudo`；seeed-reserver 在 sudo 组，密码 0。
- 将真实的本地 TPU-MLIR 安装挂载到容器的 `/workspace/tpu-mlir`（宿主机实际位置 `/home/seeed0/tpu-workspace/tpu-mlir`）；镜像的内置路径是空的，必须挂载。
- 使用 `model_transform.py`、`run_calibration.py` 和 `model_deploy.py`。
- 将工作保持在固态盘的任务目录下，如 `/home/seeed0/tpu-workspace/recamera_<model>`（`tpu-workspace` 挂载进容器即 `/workspace`；转换工作区在固态盘，NTFS 机械盘 `/media/seeed/新加卷` 不放转换工作区、build 目录，只放大件数据）。

命令模板：

```bash
cd /home/seeed0/tpu-workspace/recamera_<model>   # 宿主机工作目录 = 容器内 /workspace/recamera_<model>
sudo docker run --rm \
  -v /home/seeed0/tpu-workspace:/workspace \
  sophgo/tpuc_dev:v3.1 bash -lc '
    source /workspace/tpu-mlir/envsetup.sh
    cd /workspace/recamera_<model>
...
- 在 reCamera 上保存帧，将其拉回到 seeed-reserver（scp/sshpass），然后使用 `ffmpeg` 编码。
...
在 seeed-reserver 上构建（`~/.bashrc`/`~/.profile` 已写入这些变量，交互式 shell 生效；脚本/非交互命令中仍应显式写完整路径 export）：

```bash
export SG200X_SDK_PATH=/home/seeed0/toolchain/sg2002_recamera_emmc
export PATH=/home/seeed0/toolchain/host-tools/gcc/riscv64-linux-musl-x86_64/bin:$PATH
cd /home/seeed0/sscma-example-sg200x/solutions/sesg-project/<demo_name>
...
5. 恢复服务/重启并验证 `http://192.168.42.112/api/version`。
...
| `/workspace/tpu-mlir` 为空 | 本地 TPU-MLIR 未挂载 | `-v /home/seeed0/tpu-workspace:/workspace`，TPU-MLIR 应在 `/home/seeed0/tpu-workspace/tpu-mlir` |
...
8. 必须挂载本地 tpu-mlir 到 Docker（`-v /home/seeed0/tpu-workspace:/workspace`）。
