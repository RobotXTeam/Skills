...
> 2026-08-13 起部署主机迁移至 seeed-reserver（ae-reserver skill），此前记录产生于旧部署主机（seeed）。迁移后的新记录写入旧主机名一律指 seeed-reserver。
...
3. 识别模型数据集并准备共享缓存
4. 准备固定评测输入
5. 在 seeed-reserver 主机运行官方 Python baseline（seeed-reserver 无 NVIDIA GPU，纯 CPU 推理；demo 以 CNN 为主，CPU 完全可跑，无需 GPU 主机）
6. 部署到 reCamera
