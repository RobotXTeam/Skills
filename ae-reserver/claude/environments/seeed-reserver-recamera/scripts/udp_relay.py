#!/usr/bin/env python3
"""UDP datagram relay (seeed-reserver side) — fallback when the device is NOT on the same
...
UDP is not possible (device reachable only via seeed-reserver USB, or routed path has a
...
Run on seeed-reserver:
  python3 udp_relay.py <local_recv_port> <target_host> <target_port>
Example:
  python3 udp_relay.py 9100 192.168.2.101 9200
  # device sender targets seeed-reserver's USB IP:<local_recv_port> (discover seeed-reserver's
  # USB IP on seeed-reserver with scripts/reserver_usb_ip.sh)
"""
import socket
import sys
