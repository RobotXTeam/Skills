...
ssh_cmd="$skill_dir/environments/seeed-reserver-recamera/scripts/recamera_ssh.sh"

echo "== seeed-reserver =="
sshpass -p "${SEEED_PASSWORD:-0}" ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=8 "${SEEED_ALIAS:-seeed-reserver}" \
  'hostname; whoami; ip -4 -br addr | sed -n "1,20p"; echo; curl -I --connect-timeout 3 --max-time 5 http://192.168.42.112 2>/dev/null | sed -n "1,8p"; nc -vz -w 3 192.168.42.112 22 2>&1 || true'
