...
数据集缓存位置固定为 seeed-reserver 的 `~/reCamera_demo/datasets/`（软链接 → 机械盘工作产物根目录 `/media/seeed/新加卷/reCamera/reCamera_demo`，所有 demo 共享复用）：
...
如果 seeed-reserver（CPU 推理）Python baseline 在当前输入上本身效果明显不好，必须先回到本步骤更换或补充更可信的评测输入，不能直接用低可信 baseline 判定 reCamera 效果失败。
...
在 seeed-reserver 上 `~/reCamera_demo` 是指向机械盘工作产物根目录 `/media/seeed/新加卷/reCamera/reCamera_demo` 的软链接；评测输入/结果等大件数据放这里，可执行程序与 build 目录必须放固态盘（仓库内）。

要求：
- 同一批输入必须同时用于 seeed-reserver 主机（CPU 推理）Python baseline 和 reCamera 推理。seeed-reserver 无 NVIDIA GPU、CPU 推理较慢，评测输入应尽量精简，避免过长视频或过大图片批量。
...
### 5. 在 seeed-reserver 主机运行官方 Python baseline

在 seeed-reserver 主机上使用开源项目官方 Python 示例或官方推荐推理脚本跑同一批固定输入，作为 teacher reference。seeed-reserver 没有 NVIDIA GPU（只有 Intel UHD 核显，Python 3.12.3 系统级、无 conda），baseline 一律纯 CPU 推理：demo 以 CNN 为主，CPU 完全可以跑，无需 GPU 主机；CPU 推理较慢，超大输入（长视频/大尺寸）适当控制帧数避免耗时过长。这个结果不是绝对真值，但它代表原始模型在标准 CPU/Python 链路下的预期效果。
...
- 录制视频窗口。
- 传回本机。
- 录制视频流和图片必须要有足够的证据，比如实时 OSD 检测框或者 OSD 数据在画面上，或者导出来在 seeed-reserver 主机上叠加。用户看到视频后应能直接理解这个 demo 的视觉效果。
...
复用 `knowhubs/reCamera_KnowHub/live-osd-display.md` 的能力三件套（`environments/seeed-reserver-recamera/scripts/` 下 `udp_sender.py` / `udp_viewer.py` / `udp_relay.py`）。前提：demo 在设备上持续往一个目录写 `frame_NNNN.jpg`（带 OSD 的标注帧）。
...
#    不同子网(只能经 seeed-reserver USB): seeed-reserver 起 udp_relay.py <reserver_usb_ip>:9100 -> 本机:9200，sender 指向 reserver_usb_ip:9100
#    seeed-reserver USB 动态 IP 用 environments/seeed-reserver-recamera/scripts/reserver_usb_ip.sh 在 seeed-reserver 上探测，禁止硬编码 192.168.42.x 旧值
```
...
- 所有 rclone 命令在 **Steven 本机**执行（rclone 与 `agent:` remote 只配置在 Steven 本机，seeed-reserver 未安装 rclone）。
- 上传命令（登录验证、mkdir 四目录、带 `--filter` 的 run/模型/图片/视频上传、lsf 核对、curl 访问验证）按 `environments/seeed-reserver-recamera/network.md` 的"云端资产发布"一节执行。
- `run/` 开箱即跑包的内容规格按 `environments/seeed-reserver-recamera/development-policy.md` 的"run/ Ready-to-Run Package (Core Requirement)"一节准备。
...
用户确认 demo 和文档无误后，在 seeed-reserver 上（仓库 `/home/seeed0/sscma-example-sg200x`）提交并推送：seeed-reserver GitHub 直连可用、无需任何代理（seeed-reserver 上没有 Clash，不要配置 git http.proxy；gh CLI 已登录并完成 setup-git）→ `git pull origin main` → 复制 `<demo_name>_Demo_Wiki.md`、`DEPLOY_REPORT.md`、`eval/qa_report.md` 到 `solutions/sesg-project/<demo_name>/wiki/`，复制 1-3 张关键证据帧到 `evidence/` → `git add` → `git commit` → `git push origin main`。复制-推送命令序列参考 `knowhubs/reCamera_KnowHub/capability-map.md` 的"同步流程"。
...
- GitHub 提交内容与 Drive 上传内容的划分，见本文末尾"推送到 GitHub 的内容"表和 `environments/seeed-reserver-recamera/network.md` 的"GitHub 仓库"一节

### 17. GitHub 干净克隆验证闭环

推送到 GitHub 后，**不要立即把 Wiki 当作最终完成**。必须在固定测试线/干净验证目录中拉取刚推送的 GitHub 版本，验证外部用户路径是否完整。

验证原则（干净 clone、只用公开构建命令、`file` 验证 RISC-V musl ELF、从 Drive 拉取声明资产、部署验收、全链路闭环、失败回修）见 `environments/seeed-reserver-recamera/development-policy.md` 的"Post-Push Verification Gate"和 `knowhubs/reCamera_KnowHub/project-layout.md` 的"Post-Push Clean Verification"。

参考验证脚本骨架。**注意执行位置**：clone/build/deploy 在 seeed-reserver 或固定测试线执行（clone/build 目录必须在固态盘，机械盘 NTFS 分区不放可执行程序和 build 目录）；但 `rclone copy agent:` 拉取 Drive 资产**只能在 Steven 本机**执行（rclone 与 `agent:` remote 只在本机，seeed-reserver 未安装 rclone），先拉到本机暂存目录，再传到验证环境：

```bash
# 在 seeed-reserver 或固定测试线执行；目录必须是临时干净目录（放固态盘，勿放机械盘 NTFS 分区）
...
# seeed-reserver 上 ~/.bashrc/.profile 已写入这些变量，但只对交互式 shell 生效；
# 脚本/非交互命令中必须显式 export 完整路径：
export SDK_ROOT=/home/seeed0/toolchain/sg2002_recamera_emmc        # = $SG200X_SDK_PATH，详见 environments/seeed-reserver-recamera/toolchain.md
...
# 再传到验证环境：scp/rsync <local-staging>/{run,model} 到 seeed-reserver:$VERIFY_ROOT/

# 部署到 reCamera 并按公开运行命令验收（设备一律经 seeed-reserver 访问；USB 动态 IP 用 reserver_usb_ip.sh 探测）
# 传输可以使用 environments/seeed-reserver-recamera/scripts/recamera_scp_to.sh
# 运行可以使用 environments/seeed-reserver-recamera/scripts/recamera_ssh.sh
```
...
面向外部读者的文档硬性要求：
- README、Wiki、demo 文档和项目源码中的注释/示例命令不要出现 Steven 本机或 seeed-reserver 主机的绝对路径，例如 `/home/steven/...`、`/home/steven/work/...`、`/home/steven/下载/...`、`/home/seeed0/...`。
...
- 环境配置：`environments/seeed-reserver-recamera/`
