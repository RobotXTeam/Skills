...
- 数据集统一缓存到 `/home/steven/work/reCamera_demo/datasets/`，COCO、DOTA、VOC、ICDAR、ImageNet、KITTI 等常用数据集必须复用本地缓存，避免每个 demo 重复下载。
...
- 如果 seeed NVIDIA/Python baseline 在当前固定输入上本身效果很差，必须先更换或补充更可信的评测输入，不能直接用低可信 baseline 判定 reCamera 失败。
...
5. 使用 `environments/seeed-recamera/` 部署项目到 reCamera，并用同一输入运行推理
